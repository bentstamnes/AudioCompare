"""Test causal gate + pre-scatter gate combinations on Sloppy Groove."""
import sys, os, time
import numpy as np
...
def compute(samples, n_cols, height, sample_rate, pre_gate, causal_limit, tilt_db_per_decade):
    ...
    tilt_db   = (tilt_db_per_decade * np.log10(np.maximum(freq_rows, 1.0) / spmod._TILT_F_REF)).astype(np.float32)
    ...
    for t_start in range(0, n_cols, spmod._N_COLS_MAX):
        ...
        if causal_limit < spmod._FFT_SIZE // 2:
            t_hat_offset = (np.real(X_th / denom) - spmod._FFT_SIZE // 2).astype(np.float32)
            causal       = (t_hat_offset <= causal_limit).ravel()
        else:
            causal = np.ones(mag.size, dtype=bool)   # gate disabled
...
# causal_limit >= FFT_SIZE//2 means gate fully disabled
DISABLED = spmod._FFT_SIZE   # 4096, well above FFT_SIZE//2

tests = [
    # label,                       pre_gate, causal_limit,         tilt
    ("gate0.0_causalOFF_tilt4.5",  0.0,      DISABLED,             4.5),
    ("gate0.0_causalOFF_tilt9.0",  0.0,      DISABLED,             9.0),
    ("gate0.0_causal1024_tilt9.0", 0.0,      1024,                 9.0),
]

for label, pg, cl, tilt in tests:
    t0    = time.perf_counter()
    tiles = compute(samples, n_cols, HEIGHT, sr, pg, cl, tilt)
    dt    = time.perf_counter() - t0
    full  = np.concatenate(tiles, axis=1)
    rgb   = (np.clip(full[..., :3], 0.0, 1.0) * 255).astype(np.uint8)
    prefix = _next_prefix()
    path   = os.path.join(RESULTS, f"{prefix}_{label}_scaled.png")
    img = Image.fromarray(rgb, "RGB")
    img = img.resize((WAVE_W, HEIGHT), Image.NEAREST)
    img.save(path)
    print(f"{label}  t={dt:.2f}s  -> {os.path.basename(path)}")
