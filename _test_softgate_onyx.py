"""Same soft-crossover gate test on Onyx."""
import sys, os, time
import numpy as np
...
WAV    = os.path.join(os.path.dirname(__file__), "testfiles", "Stonebank - Onyx - Spotify V3 - 28.7.23.wav")
WAVE_W = 1800
HEIGHT = 300
...
def compute_softgate(samples, n_cols, height, sample_rate,
    ...
    bin_freq  = k_arr * (sample_rate / float(spmod._FFT_SIZE))
    gate_weight = np.clip((f_off - bin_freq) / (f_off - f_full), 0.0, 1.0).astype(np.float32)
    pre_gate_per_bin   = (gate_weight * spmod._PRE_SCATTER_GATE).astype(np.float32)
    causal_limit_per_bin = (gate_weight * spmod._CAUSAL_GATE_SAMPLES + (1.0 - gate_weight) * spmod._FFT_SIZE).astype(np.float32)
...
for tilt in [4.5, 6.0, 9.0]:
    ...
    prefix = _next_prefix()
    path   = os.path.join(RESULTS, f"{prefix}_onyx_softgate_tilt{tilt:.1f}_scaled.png")
