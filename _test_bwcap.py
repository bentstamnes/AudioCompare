"""Quick headless test: render Onyx at three bandwidth-cap values and save PNGs."""
import sys, os, time
import numpy as np
...
WAV    = os.path.join(HERE, "testfiles", "Stonebank - Onyx - Spotify V3 - 28.7.23.wav")
WAVE_W = 1800
HEIGHT = 300
RESULTS = os.path.join(HERE, "_test_spectro_results")
...
n_samp  = len(samples)
print(f"n_cols={n_cols}  sr={sr}  n_samp={n_samp}")
...
def compute_with_cap(samples, n_cols, height, sample_rate, bw_cap):
    """Identical to compute_static but _half_bw_k is capped at bw_cap rows."""
    ...
    tilt_db   = (
        spmod._TILT_DB_PER_DECADE * np.log10(np.maximum(freq_rows, 1.0) / spmod._TILT_F_REF)
    ).astype(np.float32)

    # Bandwidth precompute WITH CAP
    ...
    row_upper_k = (np.log(spmod._FS_FREQ_HI / f_hi_k) / spmod._LOG_RANGE * (height - 1)).astype(np.float32)
    row_lower_k = (np.log(spmod._FS_FREQ_HI / f_lo_k) / spmod._LOG_RANGE * (height - 1)).astype(np.float32)
    half_bw_k   = np.clip((row_lower_k - row_upper_k) / 2.0, 0.5, float(bw_cap)).astype(np.float32)
    ...
    for t_start in range(0, n_cols, spmod._N_COLS_MAX):
        ...
        omega_hat = (k_bins - np.imag(ratio).astype(np.float32) * (spmod._FFT_SIZE / (2.0 * np.pi)))
        omega_hat = np.clip(omega_hat, 0.0, float(spmod._N_BINS - 1))
        ...
        pre_gate   = (mag > col_peak_l * spmod._PRE_SCATTER_GATE).ravel()
        valid      = (mag > _MAG_GATE).ravel() & causal & pre_gate

        freq_hat  = omega_hat * (sample_rate / float(spmod._FFT_SIZE))
        freq_hat  = np.clip(freq_hat, spmod._FS_FREQ_LO, spmod._FS_FREQ_HI)
        t_row_v   = np.log(spmod._FS_FREQ_HI / freq_hat) / spmod._LOG_RANGE
        row_f     = (t_row_v * (height - 1)).astype(np.float32)
        ...
        if len(valid_idx) == 0:
            ...
            out_long = np.bincount(
                flat_rep, weights=(mag_rep * weight).astype(np.float64), minlength=size
...
for cap in [1.5, 2.5, 4.0]:
    ...
    tiles = compute_with_cap(samples, n_cols, HEIGHT, sr, cap)
    dt    = time.perf_counter() - t0
    full  = np.concatenate(tiles, axis=1)
    rgb   = (np.clip(full[..., :3], 0.0, 1.0) * 255).astype(np.uint8)
    prefix = _next_prefix()
    path   = os.path.join(RESULTS, f"{prefix}_bwcap{cap:.1f}_scaled.png")
    save_png(rgb, path, scale_w=WAVE_W)
    print(f"cap={cap:.1f}  t={dt:.2f}s  saved: {path}")
