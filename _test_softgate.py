"""Option B: soft crossover gates — full strength at <=500Hz, zero at >=4kHz."""
import sys, os, time
import numpy as np
...
def compute_softgate(samples, n_cols, height, sample_rate,
                     f_full=500.0, f_off=4000.0, tilt=4.5):
    """
    Gates fade linearly from full-strength at f_full Hz to zero at f_off Hz.
    - pre_scatter_gate fades: 0.05 → 0.0
    - causal_gate limit fades: _CAUSAL_GATE_SAMPLES → FFT_SIZE (effectively disabled)
    """
    ...
    tilt_db   = (tilt * np.log10(np.maximum(freq_rows, 1.0) / spmod._TILT_F_REF)).astype(np.float32)
    ...
    # Per-bin gate strength: 1.0 at f<=f_full, 0.0 at f>=f_off, linear between
    bin_freq     = k_arr * (sample_rate / float(spmod._FFT_SIZE))  # nominal Hz per bin
    gate_weight  = np.clip((f_off - bin_freq) / (f_off - f_full), 0.0, 1.0).astype(np.float32)
    # (N_BINS,) — 1.0 = full gate, 0.0 = gate off

    # Per-bin pre-scatter threshold (shape N_BINS)
    pre_gate_per_bin = (gate_weight * spmod._PRE_SCATTER_GATE).astype(np.float32)  # (N_BINS,)

    # Per-bin causal limit: tight at low freq, FFT_SIZE (disabled) at high freq
    causal_limit_per_bin = (
        gate_weight * spmod._CAUSAL_GATE_SAMPLES +
        (1.0 - gate_weight) * spmod._FFT_SIZE
    ...
    for t_start in range(0, n_cols, spmod._N_COLS_MAX):
        ...
        mag  = (np.abs(X_h) / spmod._FFT_NORM).astype(np.float32)  # (t_ncols, N_BINS)
        ...
        t_hat_offset = (np.real(X_th / denom) - spmod._FFT_SIZE // 2).astype(np.float32)  # (t_ncols, N_BINS)
        # Per-bin causal gate: compare each bin's t_hat against its own limit
        causal = (t_hat_offset <= causal_limit_per_bin[None, :]).ravel()

        # Per-bin pre-scatter gate: threshold scales with bin frequency
        col_peak_l = mag.max(axis=1, keepdims=True)          # (t_ncols, 1)
        pg = (mag > col_peak_l * pre_gate_per_bin[None, :]).ravel()
...
# Three variants: tilt stays at 4.5 (unchanged), 6.0, and 9.0
for tilt in [4.5, 6.0, 9.0]:
    t0    = time.perf_counter()
    tiles = compute_softgate(samples, n_cols, HEIGHT, sr, tilt=tilt)
    dt    = time.perf_counter() - t0
    full  = np.concatenate(tiles, axis=1)
    rgb   = (np.clip(full[..., :3], 0.0, 1.0) * 255).astype(np.uint8)
    prefix = _next_prefix()
    path   = os.path.join(RESULTS, f"{prefix}_softgate_tilt{tilt:.1f}_scaled.png")
    Image.fromarray(rgb, "RGB").resize((WAVE_W, HEIGHT), Image.NEAREST).save(path)
    print(f"tilt={tilt:.1f}  t={dt:.2f}s  -> {os.path.basename(path)}")
